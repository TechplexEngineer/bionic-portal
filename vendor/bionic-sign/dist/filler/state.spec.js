...
import { cloneValues, createFillerState, firstInvalidField, requiredProgress, setSignatureValue, setTextValue, submissionValues, validateFiller } from './state.js';
function definition() {
    return {
        version: 1,
        fields: [
...
describe('filler state commands', () => {
    ...
    it('stores each signature independently without mutating prior values or inputs', () => {
        const first = { type: 'signature', image: 'data:image/png;base64,Zmlyc3Q=' };
        const second = { type: 'signature', image: 'data:image/png;base64,c2Vjb25k' };
        const empty = {};
    ...
    it.each([true, false])('treats a prototype-named %s field as absent unless it is an own value', (required) => {
