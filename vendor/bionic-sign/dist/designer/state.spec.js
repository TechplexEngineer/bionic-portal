...
import { MIN_FIELD_SIZE, addField, constrainMovedFieldRect, deleteField, renameField, toggleRequired, updateDropdownOptions, updateFieldRect } from './state.js';
...
describe('designer state commands', () => {
    it('adds required fields with stable IDs and the first available generated name', () => {
        ...
        expect(first.fields[1]).toEqual(expect.objectContaining({ name: 'text_2', type: 'text', page: 2, required: true }));
    ...
    it('adds dropdown fields with a configurable default option', () => {
        ...
        expect(added.fields[0]).toEqual(expect.objectContaining({
            name: 'dropdown_1',
            type: 'dropdown',
            options: ['Option 1']
        }));
