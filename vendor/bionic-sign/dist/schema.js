...
function readFiniteNumber(value, description) {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        throw new TypeError(`${description} must be a finite number`);
    }
    return value;
}
function readRect(value, fieldName) {
    if (!isRecord(value)) {
...
function readField(value, index) {
    ...
    if (!FIELD_NAME_PATTERN.test(name)) {
        throw new TypeError(`Field "${name}" name must start with a letter and contain only letters, numbers, underscores, or hyphens`);
    }
...
export function nextFieldName(type, fields) {
    let index = 1;
...
export function cloneDefinition(definition) {
    return {
        ...
        fields: definition.fields.map((field) => field.type === 'dropdown'
            ? { ...field, rect: { ...field.rect }, options: [...field.options] }
            : { ...field, rect: { ...field.rect } })
    };
}
function diagnostic(code, message, fieldName) {
    return { code, message, fieldName };
}
export function applyTextPrefill(definition, prefill) {
    ...
    for (const [name, value] of Object.entries(prefill)) {
        ...
        if (!field) {
            diagnostics.push(diagnostic('unknown-prefill-field', `No field named "${name}" exists`, name));
            continue;
        }
        if (field.type !== 'text') {
            diagnostics.push(diagnostic('invalid-prefill-type', `Field "${name}" is not a text field`, name));
            continue;
        }
