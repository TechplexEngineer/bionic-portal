...
const signaturePadMock = vi.hoisted(() => {
    ...
    class FakeSignaturePad extends EventTarget {
        ...
        toDataURL = vi.fn(() => this.canvas.dataset.image ?? 'data:image/png;base64,c2lnbmF0dXJl');
        data = [];
        empty = true;
        constructor(canvas) {
            super();
...
async function renderReady(props = {}) {
    ...
    await vi.waitFor(() => expect(signaturePadMock.FakeSignaturePad.instances.length).toBeGreaterThanOrEqual(1));
    return result;
}
async function waitForImageLoadStarted(image) {
    await vi.waitFor(() => expect(signaturePadMock.FakeSignaturePad.instances.some((instance) => instance.fromDataURL.mock.calls.some(([loadedImage]) => loadedImage === image))).toBe(true));
}
function imageLoader(image) {
    const loader = signaturePadMock.FakeSignaturePad.instances.find((instance) => instance.fromDataURL.mock.calls.some(([loadedImage]) => loadedImage === image));
    if (!loader)
        throw new Error(`No signature-pad image loader found for ${image}`);
    return loader;
}
function firstPixel(canvas) {
...
describe('SignaturePad', () => {
    ...
    it('keeps the committed image visible across a deferred A-to-B-to-A replacement', async () => {
        ...
        const canvas = result.container.querySelector('canvas');
        if (!canvas)
            throw new Error('Expected a signature canvas');
        await waitForImageLoadStarted(first.image);
        firstGate.resolve();
