...
describe('PdfFormDesigner', () => {
    ...
    it.each(['duplicate-id', 'duplicate-name', 'malformed-rect'])('reports and recovers from a %s definition at the prop boundary', async (kind) => {
        usePdf();
        const onerror = vi.fn();
        ...
        expect(onerror).toHaveBeenCalledWith(expect.objectContaining({ code: 'invalid-form-definition' }));
    ...
    it('rejects fields beyond the loaded document page count and recovers after correction', async () => {
        ...
        await vi.waitFor(() => expect(onerror).toHaveBeenCalledWith(expect.objectContaining({ code: 'definition-page-out-of-range' })));
        await expect.element(page.getByRole('button', { name: 'Add text field' })).toBeDisabled();
    ...
    it('composes the three panes, selects pages, and adds immediately selected fields immutably', async () => {
        ...
        const ondefinitionchange = vi.fn((next) => {
        ...
        expect(pdfMocks.loadPdfBytes).toHaveBeenCalledWith(source, expect.objectContaining({ requestInit }));
        ...
        expect(emitted.at(-1)?.fields.at(-1)).toEqual(expect.objectContaining({ name: 'text_1', page: 2, type: 'text', required: true }));
    ...
    it('adds a dropdown and configures its string options', async () => {
        ...
        expect(changes.at(-1)?.fields[0]).toEqual(expect.objectContaining({
            type: 'dropdown',
    ...
    it('edits every property, rejects duplicate names inline, deletes, and preserves geometry across zoom', async () => {
        ...
        expect(overlay.style.left).toBe('62.5px');
    ...
    it('keeps zoomed PDF overflow inside the center document pane', async () => {
        ...
        const designer = result.container.querySelector('.pdf-form-designer');
        ...
        await vi.waitFor(() => expect(documentPane.scrollHeight).toBeGreaterThan(documentPane.clientHeight));
    ...
    it('provides responsive drawer controls and customizable toolbar, loading, and error snippets', async () => {
        ...
        const error = createRawSnippet((reason) => ({
        ...
        const fieldsPane = result.container.querySelector('[aria-label="Fields and pages"]');
