...
describe('loadPdfBytes', () => {
    ...
    it('forwards RequestInit and the operation signal for URL sources', async () => {
        ...
        const fetch = vi.fn().mockResolvedValue(new Response(responseBytes, {
            status: 200
        }));
        vi.stubGlobal('fetch', fetch);
    ...
    it.each(['operation', 'request'])('combines signals so the %s signal can cancel an active URL load', async (cancelledSignal) => {
    ...
    it('maps non-OK URL responses to a stable typed error', async () => {
        ...
        const error = await loadPdfBytes('https://example.test/missing.pdf').catch((reason) => reason);
    ...
    it('propagates abort failures without mapping them to network errors', async () => {
        ...
        const error = await loadPdfBytes('https://example.test/form.pdf').catch((reason) => reason);
