...
import { decodePDFRawStream, degrees, PDFArray, PDFContentStream, PDFDict, PDFDocument, PDFName, PDFRawStream } from 'pdf-lib';
import { afterEach, beforeAll, describe, expect, it, vi } from 'vitest';
import { BionicSignError } from '../types.js';
import { exportFlattenedPdf } from './export.js';
const TWO_BY_ONE_PNG = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAABCAYAAAD0In+KAAAAFElEQVR42mP8z8Dwn4GBgYGJAQoAHgAD/VoS8AAAAABJRU5ErkJggg==';
const ONE_BY_ONE_PNG = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2+XkAAAAASUVORK5CYII=';
const NON_CANONICAL_ONE_BY_ONE_PNG = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2+XkAAAAASUVORK5CYIJ=';
let fixture;
beforeAll(async () => {
    fixture = new Uint8Array(await readFile(new URL('../template/fixtures/permission-form.pdf', import.meta.url)));
});
function definition(fields) {
    return { version: 1, fields };
}
function contentForPage(document, pageIndex) {
...
function matrixValues(match) {
    return match.slice(1).map(Number);
}
function imageTransformBlocks(content) {
    return [...content.matchAll(/q\s+([\s\S]*?)\/\S+ Do\s+Q/g)].map((block) => [
        ...block[1].matchAll(/(-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+) cm/g)
    ].map(matrixValues));
}
function imageMatrices(content) {
...
function textMatrices(content) {
    return [
        ...content.matchAll(/(-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+) Tm/g)
    ].map(matrixValues);
}
async function sourceWithRotation(rotation) {
    const document = await PDFDocument.create();
    const page = document.addPage([612, 792]);
    page.setRotation(degrees(rotation));
    return document.save();
}
async function sourceWithVisibleBox(rotation) {
    const document = await PDFDocument.create();
...
function imageResourceCount(document, pageIndex) {
...
describe('exportFlattenedPdf', () => {
    ...
    it('returns a new parseable PDF with text on its defined page without mutating inputs', async () => {
        ...
        const values = { student_name: { type: 'text', value: 'Jordan Lee' } };
    ...
    it.each([
    ...
    ])('rejects a %s required value', async (_description, type, value) => {
        const form = definition([
            {
                id: 'required-id',
                name: 'required_field',
        ...
        const values = value ? { required_field: value } : {};
        const error = await exportFlattenedPdf(fixture, form, values).catch((reason) => reason);
    ...
    it('maps a runtime null value to the typed value-mismatch error', async () => {
        ...
        const runtimeValues = { name: null };
        const error = await exportFlattenedPdf(fixture, form, runtimeValues).catch((reason) => reason);
        expect(error).toBeInstanceOf(BionicSignError);
        expect(error).toMatchObject({ code: 'export-value-type' });
    });
    it.each([true, false])('treats an inherited prototype value as missing for a %s prototype-named field', async (required) => {
        const form = definition([
            {
                id: 'prototype-id',
                name: 'toString',
        ...
        const result = await exportFlattenedPdf(fixture, form, {}).catch((reason) => reason);
        if (required) {
            ...
            expect(await PDFDocument.load(result)).toBeDefined();
        }
    });
    it.each([0, 1.5, 3])('rejects a field page outside the source PDF: %s', async (fieldPage) => {
        ...
        const error = await exportFlattenedPdf(fixture, form, {}).catch((reason) => reason);
        expect(error).toBeInstanceOf(BionicSignError);
    ...
    ])('orients text with the visual viewport on a %i-degree page', async (rotation, expectedMatrix) => {
    ...
    it.each([
    ...
    ])('places text and signatures against the PDF.js-visible crop box on a %i-degree page', async (rotation, expectedText, expectedImageTranslation, expectedImageRotation) => {
        ...
        expect(image[0][0]).toEqual(expectedImageTranslation.map((value) => expect.closeTo(value, 5)));
    ...
    it.each([
    ...
    ])('orients and aspect-fits a signature in the visual viewport on a %i-degree page', async (rotation, expectedTranslation, expectedRotation, expectedScale) => {
        const source = await sourceWithRotation(rotation);
        const form = definition([
            {
        ...
        expect(transforms[0][0]).toEqual(expectedTranslation.map((value) => expect.closeTo(value, 5)));
    ...
    it('shrinks text below 12pt while keeping it at or above 8pt when it fits', async () => {
        ...
        const document = await PDFDocument.load(output);
        const sizes = [...contentForPage(document, 0).matchAll(/\/Helvetica-\d+ ([\d.]+) Tf/g)].map((match) => Number(match[1]));
    ...
    ])('rejects text that cannot fit the field %s at 8pt', async (_axis, rect, value) => {
        const form = definition([
            {
                id: 'overflow-id',
                name: 'overflow',
    ...
    it('propagates an already-aborted signal reason without changing it', async () => {
        ...
        const error = await exportFlattenedPdf(fixture, definition([]), {}, {
    ...
    it('observes an abort that occurs while PDF loading is in flight', async () => {
        ...
        const promise = exportFlattenedPdf(fixture, definition([]), {}, {
            signal: controller.signal
        });
        controller.abort(reason);
    ...
    it('maps a generic pdf-lib load failure to a stable typed error', async () => {
        const error = await exportFlattenedPdf(new Uint8Array([1, 2, 3]), definition([]), {}).catch((reason) => reason);
        expect(error).toBeInstanceOf(BionicSignError);
    ...
    it('maps an encrypted pdf-lib load failure to the shared encrypted-PDF code', async () => {
        ...
        const error = await exportFlattenedPdf(fixture, definition([]), {}).catch((reason) => reason);
        expect(error).toBeInstanceOf(BionicSignError);
